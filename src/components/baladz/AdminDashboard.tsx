...
import { AdminPendaftarDetailModal } from "./AdminPendaftarDetailModal";
...
export function AdminDashboard() {
  ...
  // State Modal Detail & Verifikasi PSB Calon Santri
  const [detailModalPendaftar, setDetailModalPendaftar] = useState<PendaftarRow | null>(null);
  ...
  const openDetailModal = (pendaftar: PendaftarRow) => {
    setDetailModalPendaftar(pendaftar);
  };

  const handleUpdatePendaftarPSB = async (pId: number, updates: Partial<PendaftarRow>) => {
    try {
      const res = await fetch("/api/pendaftar", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: pId, ...updates }),
      });
      const data = await res.json();
      if (data.success) {
        showToast("Data pendaftar berhasil diperbarui");
        await fetchPendaftar();
        if (detailModalPendaftar && detailModalPendaftar.id === pId) {
          setDetailModalPendaftar((prev) => (prev ? { ...prev, ...updates } : null));
        }
      } else {
        alert(data.error || "Gagal memperbarui data");
      }
    } catch (err) {
      console.error("Error updating pendaftar PSB:", err);
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F2] text-stone-800 font-sans">
          ...
          <main className="flex-1 min-w-0 w-full space-y-6">
                              ...
                              >
                                ...
                                <td className="py-3.5 px-4 align-top">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span className="font-bold text-stone-900 text-sm leading-snug">{item.nama_santri}</span>
                                    {item.no_pendaftaran && (
                                      <span className="font-mono text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-900 border border-emerald-200">
                                        {item.no_pendaftaran}
                                      </span>
                                    )}
                                  </div>
                                ...
                                >
                                  <div className="flex items-center justify-end gap-2">
                                    ...
                                    <button
                                      ...
                                      title="Kelola berkas, verifikasi pembayaran, seleksi & pengumuman"
                                    >
                                      <FileCheck2 className="w-3.5 h-3.5 shrink-0 text-amber-300" />
                          ...
                          >
                            ...
                            <div className="pt-2 border-t border-stone-100 flex flex-col gap-2">
                              <button
                                type="button"
                                onClick={() => openDetailModal(item)}
                                className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-bold text-white bg-[#0F4C3A] hover:bg-[#0c3f30] shadow-xs transition cursor-pointer active:scale-[0.98]"
                              >
                                <FileCheck2 className="w-3.5 h-3.5 text-amber-300" />
                                <span>Kelola & Verifikasi PSB</span>
                              </button>
                                ...
                                {item.status === "Diterima" ? (
                                  cleanWa && diterimaWaUrl ? (
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noreferrer"
                                      className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-bold text-white bg-[#0F4C3A] hover:bg-[#0c3f30] shadow-xs transition-all active:scale-[0.98]"
                                    >
                                      <MessageCircle className="w-4 h-4" />
                                      <span>Kirim WA</span>
                                    </a>
                                  ) : (
                                    <span className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-stone-400 bg-stone-100 border border-stone-200">
                                  ...
                                  cleanWa ? (
                                    <button
                                      type="button"
                                      onClick={() => setSeleksiModalItem({ pendaftar: item, jadwal: item.jadwal_seleksi || "" })}
                                      className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-bold text-white bg-blue-700 hover:bg-blue-800 shadow-xs transition-all active:scale-[0.98] cursor-pointer"
                                    >
                                      <MessageCircle className="w-4 h-4" />
            ...
            {activeMenu === "psb" && (
              <div className="w-full min-w-0 max-w-full bg-white p-6 sm:p-8 rounded-2xl border border-stone-200 shadow-2xs space-y-6">
                ...
                {/* Template Dokumen Surat Kesanggupan PSB */}
                <div className="pt-4 border-t border-stone-200">
                  <div className="mb-3">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-[#D97706] block">
                      Dokumen Pendaftaran Calon Santri
                    </span>
                    <h3 className="font-bold text-stone-800 text-sm mt-0.5">
                      Template Surat Kesanggupan Orang Tua / Wali
                    </h3>
                    <p className="text-xs text-stone-500 mt-1 leading-relaxed">
                      Unggah dokumen format resmi Surat Kesanggupan (format PDF atau DOCX/Word). Calon santri & wali santri dapat langsung mengunduh file ini dari menu Berkas di dashboard pendaftar tanpa perlu meminta manual via WhatsApp.
                    </p>
                  </div>

                  <div className="bg-stone-50 p-4 rounded-xl border border-stone-200">
                    <SantriFileUploader
                      label="Unggah File Template Resmi (PDF / DOCX / Word)"
                      description="File ini akan langsung menjadi tautan unduh di dashboard semua akun calon santri (maksimal ukuran 8 MB)."
                      endpoint="documentUploader"
                      acceptTypes="document"
                      value={draft.psb.templateSuratKesanggupanUrl || ""}
                      onChange={(url) =>
                        setDraft({
                          ...draft,
                          psb: {
                            ...draft.psb,
                            templateSuratKesanggupanUrl: url,
                          },
                        })
                      }
                    />

                    {draft.psb.templateSuratKesanggupanUrl && (
                      <div className="mt-3 pt-3 border-t border-stone-200 flex flex-wrap items-center justify-between gap-2 text-xs">
                        <span className="text-emerald-800 font-semibold flex items-center gap-1.5">
                          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                          <span>File template aktif & langsung dapat diunduh oleh calon santri</span>
                        </span>
                        <a
                          href={draft.psb.templateSuratKesanggupanUrl}
                          target="_blank"
                          rel="noreferrer"
                          download
      ...
      {/* Modal Detail & Verifikasi PSB Calon Santri */}
      {detailModalPendaftar && (
        <AdminPendaftarDetailModal
          pendaftar={detailModalPendaftar}
          isOpen={Boolean(detailModalPendaftar)}
          onClose={() => setDetailModalPendaftar(null)}
          onUpdatePendaftar={handleUpdatePendaftarPSB}
